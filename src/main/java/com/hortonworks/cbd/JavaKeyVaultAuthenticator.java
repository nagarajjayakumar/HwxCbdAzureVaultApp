...
public class JavaKeyVaultAuthenticator {
	...
	private static Logger logger = LoggerFactory.getLogger(JavaKeyVaultAuthenticator.class);
	/**
	...
	public static KeyCert readPfx(String path, String password) throws NoSuchProviderException, KeyStoreException,
		...
		try (FileInputStream stream = new FileInputStream(path)) {
			...
			if (isAliasWithPrivateKey) {
				...
				logger.debug("the alias is: " + alias);
				...
				logger.debug("key in primary encoding format is: " + key.getEncoded());
			}
			return keyCert;
		}
	}
